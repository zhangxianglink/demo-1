...
2. 事件捕获与冒泡
   ...
   （1）捕获阶段：当用户点击按钮，浏览器会从 window 从上向下遍历至用户点击的按钮，逐个触发事件处理函数；
   （2）目标阶段：真正的目标节点正在处理事件的阶段；
   （3）冒泡阶段：浏览器从用户点击的按钮从下往上遍历至 window，逐个触发事件处理函数。
...
3. 事件委托
   ...
   ```javascript
   将监听函数定义在祖先元素（如父元素、爷爷元素）上, 由父节点在冒泡阶段统一执行(一个或者多个)子元素事件,
   好处是：1 节约监听数量 2 可以监听动态生成的元素。3.起到节省内存的效果.
